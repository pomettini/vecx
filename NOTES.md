...
- Current log format is one line every 5 seconds:
  `vecx bench build="..." window_ms=... updates=... renders=... avg_fps=... avg_render_fps=... avg_update_ms=... avg_render_ms=... min_update_ms=... max_update_ms=... emu_cycles=... emu_instr=... avg_instr_update=... avg_cpi=... avg_vectors=... wait_skips=... wait_skip_cycles=... skipped=...`
...
| Playdate C build 34 | PDU1-Y024621 | pending | pending | pending | pending | Recovery build removing DWT profiling while keeping build 32's smaller vector/hash storage. Pushed 2026-06-01 04:52 CEST; `pdex.bin` is 17316 bytes; BSS is 105884 bytes. |

## Next Steps

- Test build 34 to confirm the crash is gone. Future profiling should use safer coarse build comparisons or Playdate-supported timing APIs, not raw DWT registers.
- Full-speed emulation target is still far away: build 26 improves visible cadence, but the Playdate still only gets through roughly 327k-330k emulated cycles/sec of 1.5MHz Vectrex time.
